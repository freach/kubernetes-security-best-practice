...
**Why?**
In a private topology all K8s masters and nodes have only private IP addresses assigned. This greatly reduces the risk of exposing anything critical to the public by mistake or lack of knowledge. Also experiences shows if you need to expose ports explicitly you're more conscience of the potential consequences.
...
**Why?** SSH is a critical service which is under constant attack if exposed to the public. Not always key base authentication is used and dictionary attacks or exploits will eventually lead to intrusion. To minimize risk a hardened bastion host is introduced and SSH blocked for public access on all other nodes.
